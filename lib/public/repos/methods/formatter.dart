...
/// Returns the first day of the month (MySQL format)
/// [monthsOffset]: 0 for current month, -1 for last month, 1 for next month, etc.
String getStartOfMonth([int monthsOffset = 0]) {
  final now = DateTime.now();
  
  final targetDate = DateTime(now.year, now.month + monthsOffset, 1);
  
  return _formatToMySQLDateTime(targetDate);
}
